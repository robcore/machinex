...
/**
 * do_generic_file_read_atomic - generic file read routine - > for atomic reads
 * @filp:	the file to read
 * @ppos:	current file position
 * @desc:	read_descriptor
 * @actor:	read method
...
int file_read_iter_actor_atomic(read_descriptor_t *desc, struct page *page,
			 unsigned long offset, unsigned long size)
{
	struct iov_iter *iter = desc->arg.data;
	unsigned long copied = 0;
 ...
 */
ssize_t
generic_file_read_iter_atomic(struct kiocb *iocb, struct iov_iter *iter, loff_t pos)
{
	struct file *filp = iocb->ki_filp;
	read_descriptor_t desc;
	ssize_t retval = 0;
	...
	desc.count = count;
	desc.error = 0;
	do_generic_file_read_atomic(filp, ppos, &desc, file_read_iter_actor_atomic);
	if (desc.written)
		retval = desc.written;
	else
		retval = desc.error;
...
EXPORT_SYMBOL(generic_file_read_iter_atomic);
/**
