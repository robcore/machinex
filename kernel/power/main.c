			   ...
			   const char *buf, size_t n)
{
	...
	if (state == PM_SUSPEND_ON) {
		...
		error = 0;
		pr_info("Suspend Sysfs Entry: On");
		goto out;
	}
...
struct fake_frequency_table {
	unsigned int	index;     /* any */
	unsigned int	frequency; /* kHz - doesn't need to be in ascending
				    * order */
};

static struct fake_frequency_table fake_cpu_table[] = {
	{ .frequency = 384000 },
	{ .frequency = 486000 },
	{ .frequency = 594000 },
	{ .frequency = 702000 },
...
{
	unsigned int fakemax = 1890000;
	return fakemax;
}

int get_min_freq(void)
{
	unsigned int fakemin = 384000;
	return fakemin;
}
...
{
	int i;
	struct fake_frequency_table *table;

	table = fake_cpu_table;
	if (table == NULL)
		return -EFAULT;

	for (i = 0; table[i].frequency != FAKEFREQ_TABLE_END; i++) {
...
power_attr_ro(cpufreq_table);
#endif /* CONFIG_FAKE_DVFS */
