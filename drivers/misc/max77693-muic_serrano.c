...
#endif
	/* code added for T-Mobile charger`s detach */
	if ((info->adc == ADC_CEA936ATYPE1_CHG) && (info->vbvolt == 0)) {
		dev_info(info->dev, "%s: DETACHED T-Mobile Charger\n", __func__);
		intr = INT_DETACH;
	}
...
static void max77693_muic_init_detect(struct work_struct *work)
{
	...
	/* irq_init is delayed review comment from jryu.kim */
	ret = max77693_muic_irq_init(info);
	if (ret < 0) {
		dev_err(info->dev, "Failed to initialize MUIC irq:%d\n", ret);
		goto fail_irq;
	}
