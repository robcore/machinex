...
#define GAMMACONTROL_3_VERSION 3
...
static ssize_t gammacontrol_3_version(struct device * dev, struct device_attribute * attr, char * buf)
{
    return sprintf(buf, "%u\n", GAMMACONTROL_3_VERSION);
}
...
static DEVICE_ATTR(version, 0777 , gammacontrol_3_version, NULL);

static struct attribute *gammacontrol_3_attributes[] =
{
...
static struct attribute_group gammacontrol_3_group =
{
	.attrs  = gammacontrol_3_attributes,
};

static struct miscdevice gammacontrol_3_device =
{
	.minor = MISC_DYNAMIC_MINOR,
	.name = "gammacontrol_3",
};

static int __init gammacontrol_3_init(void)
{
    int ret;

    pr_info("%s misc_register(%s)\n", __FUNCTION__, gammacontrol_3_device.name);

    ret = misc_register(&gammacontrol_3_device);
    ...
    if (sysfs_create_group(&gammacontrol_3_device.this_device->kobj, &gammacontrol_3_group) < 0) {
	    pr_err("%s sysfs_create_group fail\n", __FUNCTION__);
	    pr_err("Failed to create sysfs group for device (%s)!\n", gammacontrol_3_device.name);
	}

    return 0;
}
late_initcall(gammacontrol_3_init);