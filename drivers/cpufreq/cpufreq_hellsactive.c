...
struct cpufreq_hellsactive_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_hellsactive_cpuinfo, cpuinfo);
...
static void cpufreq_hellsactive_timer_resched(unsigned long cpu)
{
	struct cpufreq_hellsactive_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
 ...
 * function.
 */
static void cpufreq_hellsactive_timer_start(int cpu)
{
	struct cpufreq_hellsactive_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq(
	struct cpufreq_hellsactive_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_hellsactive_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	int cpu_load;
	struct cpufreq_hellsactive_cpuinfo *pcpu =
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_hellsactive_timer_resched(data);
...
static void cpufreq_hellsactive_idle_start(void)
{
	...
	if (pcpu->target_freq != pcpu->policy->min) {
		...
		if (!pending) {
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			cpufreq_hellsactive_timer_resched(smp_processor_id());
...
static void cpufreq_hellsactive_idle_end(void)
{
	struct cpufreq_hellsactive_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		del_timer(&pcpu->cpu_timer);
		del_timer(&pcpu->cpu_slack_timer);
		cpufreq_hellsactive_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_hellsactive_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			struct cpufreq_hellsactive_cpuinfo *pjcpu;
			u64 hvt;
...
static int cpufreq_hellsactive_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_hellsactive_notifier,
};
						...
						unsigned long target_cpu, void *arg)
{
	...
	struct cpufreq_hellsactive_cpuinfo *target, *source;
...
static struct attribute *hellsactive_attributes[] = {
...
static struct attribute_group hellsactive_attr_group = {
	.attrs = hellsactive_attributes,
	.name = "hellsactive",
};

static int cpufreq_hellsactive_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_hellsactive_idle_start();
		break;
	case IDLE_END:
		cpufreq_hellsactive_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_hellsactive_idle_nb = {
	.notifier_call = cpufreq_hellsactive_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			cpufreq_hellsactive_timer_start(j);
		...
		idle_notifier_register(&cpufreq_hellsactive_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_hellsactive_idle_nb);
...
static void cpufreq_hellsactive_nop_timer(unsigned long data)
{
}
...
static int __init cpufreq_hellsactive_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_hellsactive_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_hellsactive_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_hellsactive_speedchange_task, NULL,
			       "cfhellsactive");
...
static void __exit cpufreq_hellsactive_exit(void)
{
...
module_exit(cpufreq_hellsactive_exit);
