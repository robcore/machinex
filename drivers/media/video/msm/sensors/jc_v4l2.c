...
static int jc_check_sum(void)
{
	...
	pr_debug("ISP Checksum : %x", isp_val);
	...
	if (jc_ctrl->isp_null_read_sensor_fw == true) {
		...
		pr_debug("FactArea Checksum : %x", factarea_val);
		pr_debug("ISP - FactArea Checksum : %x", isp_val-factarea_val);
...
retry:
	...
	if (err != 0 && jc_ctrl->fw_retry_cnt < 2) {
		pr_debug("checksum error!! retry fw write!!: %d", jc_ctrl->fw_retry_cnt);
...
static int jc_set_different_ratio_capture(int mode)
{
	...
	pr_debug("Entered, different ratio capture mode %d\n", mode);
...
void sensor_native_control(void __user *arg)
{
	...
	case EXT_CAM_UPDATE_FW:
		if (ctrl_info.value_1 == CAM_FW_MODE_DUMP) {
			...
			pr_debug("ISP version : %s\n", sysfs_isp_fw_str);
			pr_debug("Phone version : %s\n", sysfs_phone_fw_str);
			pr_debug("ISP FW Force Write Done!\n");
		}
		break;
	...
	case EXT_CAM_SET_FACTORY_BIN:
		pr_debug(" factory binary: %d", ctrl_info.value_1);
		if (ctrl_info.value_1 == 1)
	...
	case EXT_CAM_START_GOLF_SHOT:
		pr_debug("Golf shot start, 1/1000 shutter speed");
		jc_writeb(0x03, 0x0B, 0x18);
		break;

	case EXT_CAM_STOP_GOLF_SHOT:
		pr_debug("Golf shot stop, return normal shutter speed");
