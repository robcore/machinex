...
#define JC_LOAD_FW_MAIN_REQUEST	0
#define JC_DUMP_FW	0
	...
	u8 category, u8 byte, int *val, bool log)
{
	...
	if (err != 1) {
		pr_debug("RD category %#x, byte %#x, err %d\n",
			category, byte, err);
		return err;
	}
	...
	u32 addr, u32 unit, u32 count)
{
	...
	for (i = 0; i < unit*count; i += unit) {
		...
		if (val != 0) {
			pr_debug("failed to erase sector\n");
			return -EFAULT;
		}
	...
	/* Start Programming */
		...
		if (val != 0) {
			pr_debug("failed to program~~~~\n");
			return -EFAULT;
		}
	}
	pr_debug("jc_program_fw out ~~~~~~~~~~~\n");
	return 0;
}
#endif
#endif
...
static int jc_dump_fw(void)
{
	...
	if (IS_ERR(fp)) {
		pr_debug("failed to open %s, err %ld\n",
...
static int jc_SIO_loader(void)
{
	...
	pr_debug("start, file path %s, size %ld Bytes, count %d\n",
	...
	if (nread != fsize) {
		pr_debug("failed to read firmware file, %ld Bytes\n", nread);
		err = -EIO;
		goto out;
	}
...
static int jc_load_fw_main(void)
{
	...
	if (isp_version == 0x00) {
		if (firmware_update_sdcard == true) {
			pr_debug("start, file path %s, size %ld Bytes, count %d\n",
				JC_M9MO_FW_PATH_SD, fsize, count);
		} else {
			pr_debug("start, file path %s, size %ld Bytes, count %d\n",
				JC_M9MO_FW_PATH, fsize, count);
		}
	} else if (isp_version == 0x10) {
		if (firmware_update_sdcard == true) {
			pr_debug("start, file path %s, size %ld Bytes, count %d\n",
...
request_fw:
	if (fw_requested) {
		set_fs(old_fs);
		pr_debug("Firmware Path = %s\n", JC_FW_REQ_PATH);
		err = request_firmware(&fw, JC_FW_REQ_PATH, &jc_dev);


		if (err != 0) {
			pr_debug("request_firmware failed\n");
			err = -EINVAL;
			goto out;
		}
		count = fw->size / SZ_4K;
		pr_debug("start, size %d Bytes  count = %d\n", fw->size, count);
		buf_m9mo = (u8 *)fw->data;
	}
#endif
...
out:
	...
	err = jc_spi_write(Fbuf, fsize, txSize);

	if (err < 0) {
		pr_debug("jc_spi_write falied\n");
		goto out;
	}
out:
	if (!IS_ERR(fp))
...
static int jc_get_sensor_fw_version(void)
{
	...
	pr_debug("f/w version = %x\n", fw_ver);
	pr_debug("sensor_type = %s\n", sensor_type);
	...
	pr_debug("sensor fw : %s\n", sysfs_sensor_fw);
	pr_debug("sensor type : %s\n", sysfs_sensor_type);
	return 0;
}
	...
	if (isp_version == 0x00) {
		...
		if (IS_ERR(fp)) {
			...
			if (IS_ERR(fp)) {
				...
				pr_debug("FW File(phone) opened.\n");
			}
		} else {
			pr_debug("FW File(SD card) opened.\n");
...
request_fw:
	if (fw_requested) {
		...
		if (err != 0) {
			pr_debug("request_firmware falied\n");
			err = -EINVAL;
			goto out;
		}
		pr_debug("%s: fw->data[0] = %x, fw->data[1] = %x\n", __func__,
...
static int jc_check_fw(void)
{
	...
	pr_debug("phone ver = %s, sensor_ver = %s\n",
...
static int jc_check_sum(void)
{
	...
	do {
		...
		if (val == 0)
			break;
	} while (val == 0x04 && retries++ < JC_I2C_VERIFY);

	/* Get Checksum of ISP */
	err = jc_readw(JC_CATEGORY_FLASH, 0x0A, &val);
	pr_debug("err : %d", err);
	pr_debug("Checksum : %x", val);
...
static int jc_get_sensor_version(void)
{
	...
	pr_debug("Read Sensor Ver String : 11 byte");
...
{
	pr_debug("Compare Sensor with Phone FW version");

	if ((strncmp(sysfs_sensor_fw_str, "S13F0", 5) != 0)
	    && (strncmp(sysfs_sensor_fw_str, "O13F0", 5) != 0)) {
		pr_debug("sensor version is wrong, skip!");
		return -1;
	} else if (strncmp(sysfs_sensor_fw_str, sysfs_phone_fw_str, 1) != 0) {
		pr_debug("Another sensor module is detected.");
		return 1;
	} else
...
{
	pr_debug("Compare Sensor with ISP FW version\n");

	if ((strncmp(sysfs_isp_fw_str, "S13F0", 5) != 0)
	    && (strncmp(sysfs_isp_fw_str, "O13F0", 5) != 0)) {
		pr_debug("isp version is wrong, skip!");
		return 1;
	} else if ((strncmp(sysfs_sensor_fw_str, "S13F0", 5) != 0)
	    && (strncmp(sysfs_sensor_fw_str, "O13F0", 5) != 0)) {
		pr_debug("sensor version is wrong, skip!");
		return -1;
	} else if (strncmp(sysfs_sensor_fw_str, sysfs_isp_fw_str, 1) != 0) {
		pr_debug("Another sensor module is detected.");
		return 1;
	} else
...
{
	pr_debug("Compare ISP with Phone FW version\n");

	if ((strncmp(sysfs_isp_fw_str, "S13F0", 5) != 0)
	    && (strncmp(sysfs_isp_fw_str, "O13F0", 5) != 0)) {
		pr_debug("isp version is wrong, skip!");
		return -1;
	} else if (strncmp(sysfs_isp_fw_str, sysfs_phone_fw_str, 1) != 0) {
		pr_debug("Another sensor module is detected.");
		return -1;
	} else
...
static int jc_check_chip_erase(void)
{
	...
	pr_debug("Check chip erased or not");
	...
	jc_readw(JC_CATEGORY_FLASH, 0x0A, &val);
      ...
      if (chip_erase == 0x00) {
	  	pr_debug("FlashROM is blank");
		ret = 0;
      	} else if (chip_erase == 0x01) {
	  	pr_debug("FlashROM is NOT blank");
		ret = 1;
      	}
...
{
      pr_debug("Check sensor validation JF sensor or NOT");

	if ((strncmp(sysfs_sensor_fw_str, "S13L0", 5) == 0)
	    || (strncmp(sysfs_sensor_fw_str, "O13L0", 5) == 0)) {
             pr_debug("JA sensor is detected, fail!!");
		return 0;
	} else
	      return 1;
}
...
static int jc_check_sensor_spi_port(void)
{
	...
	pr_debug("check_spi_port : %d\n", check_spi_port);

	if (check_spi_port == 1) {
          pr_debug("SPI Port Error is detected, fail!!");
	    return 0;
	} else
	    return 1;
}
...
void jc_save_isp_debug_log1(void)
{
	...
	while (ptr < 10000) {
		...
		i = 0;
		while(i <= len) {
			/*pr_debug("### i %d, len %d, (len - i = %d)", i, len, len - i);*/
...
static int jc_set_mode(u32 mode)
{
	...
	case JC_SYSINIT_MODE:
		pr_debug("sensor is initializing\n");
		err = -EBUSY;
		break;
	...
	case JC_PARMSET_MODE:
		...
		int_factor = jc_wait_interrupt(JC_ISP_TIMEOUT);
		if (!(int_factor & JC_INT_MODE)) {
			pr_debug("parameter mode interrupt fail %#x\n",
				int_factor);
			return -ENOSYS;
		}
	...
	case JC_MONITOR_MODE:
		...
		int_factor = jc_wait_interrupt(JC_ISP_TIMEOUT);
		if (!(int_factor & JC_INT_MODE)) {
			pr_debug("monitor mode interrupt fail %#x\n",
				int_factor);
			return -ENOSYS;
		}
	...
	default:
		pr_debug("current mode is unknown, %d\n", mode);
		err = 0;/* -EINVAL; */
	}
...
static u32 jc_wait_interrupt(unsigned int timeout)
{
	...
	pr_debug("jc_wait_interrupt : jc_ctrl->isp.int_factor = %x\n",
	...
	for (i = 0 ; i < 10 ; i++) {
		if (jc_ctrl->isp.int_factor != 0x100) {
			...
			jc_readw(JC_CATEGORY_SYS,
				JC_SYS_INT_FACTOR, &jc_ctrl->isp.int_factor);
			pr_debug("jc_wait_interrupt retry[%d] : jc_ctrl->isp.int_factor = %x\n",
...
static int jc_set_sizes(void)
{
	...
	if ((jc_ctrl->preview_size != prev_preview_size) ||
		(jc_ctrl->capture_size != prev_capture_size)) {
		...
		if (jc_ctrl->capture_size != prev_capture_size) {
			pr_debug("### set capture size : 0x%x\n",
			...
			pr_debug("### capture size same as previous size : " \
						"0x%x\n", prev_capture_size);
	}
...
static int jc_set_snapshot_mode(int mode)
{
	...
	pr_debug("Entered, shot mode %d\n", mode);

	if (jc_ctrl->burst_mode == true
		&& mode == 1) {
		pr_debug("Now Burst shot capturing");
		return rc;
	}

	if (mode == 0) {
		pr_debug("Single Capture");
		do {
			jc_readb(JC_CATEGORY_CAPCTRL,
				        0x1F, &val);
			pr_debug("capture status : %d", val);
		} while (val != 0);
...
static int jc_set_touch_af_pos(int x, int y)
{
	...
	pr_debug("Entered, touch af pos (%x, %x)\n", x, y);

	if (jc_ctrl->af_mode >=	3
		&& jc_ctrl->af_mode <=6) {
		pr_debug("Now CAF mode. Return touch position setting!\n");
		return rc;
	}
	...
	pr_debug("(%x %x, %x %x)", h_x, l_x, h_y, l_y);
	if (jc_ctrl->movie_mode == true) {
		pr_debug("Movie touch af\n");
		jc_writeb(JC_CATEGORY_LENS,
				0x01, 0x07);
	} else {
		pr_debug("Normal touch af\n");
		jc_writeb(JC_CATEGORY_LENS,
				0x01, 0x02);
	}
...
static int jc_set_wb(int mode)
{
	...
	if (mode == 1) {
		...
		if (mode == 3) {
			pr_debug("Incandescent\n");
			jc_writeb(JC_CATEGORY_WB,
					JC_WB_AWB_MANUAL, 0x01);
		} else if (mode == 4) {
			pr_debug("Fluorescent\n");
			jc_writeb(JC_CATEGORY_WB,
...
static int jc_set_effect(int mode)
{
	...
	if (mode == 0) {
		...
		pr_debug("Sepia\n");
		pr_debug("Enable CFIXB and CFIXR\n");
		jc_writeb(JC_CATEGORY_MON,
...
static int jc_set_movie_mode(int mode)
{
	...
	pr_debug("Entered, movie mode %d\n", mode);

	jc_set_mode(JC_PARMSET_MODE);

	if (mode == 0) {
		pr_debug("Capture mode\n");
		jc_ctrl->movie_mode = false;
		jc_writeb(JC_CATEGORY_PARM,
...
static int jc_set_antibanding(int mode)
{
	...
	pr_debug("Entered, antibanding %d\n", mode);

	if (mode == 0) {
		pr_debug("flicker off\n");
		jc_writeb(JC_CATEGORY_AE,
				0x06, 0x05);
	} if (mode == 1) {
		pr_debug("60Hz auto flicker mode\n");
		jc_writeb(JC_CATEGORY_AE,
				0x06, 0x02);
	} if (mode == 2) {
		pr_debug("50Hz auto flicker mode\n");
		jc_writeb(JC_CATEGORY_AE,
				0x06, 0x01);
	} if (mode == 3) {
		pr_debug("auto flicker mode\n");
		jc_writeb(JC_CATEGORY_AE,
				0x06, 0x00);
	}
...
static int jc_set_shot_mode(int mode)
{
	...
	pr_debug("Entered, shot mode %d / %d\n", mode, isp_mode);
...
static int jc_set_ocr_focus_mode(int mode)
{
	...
	pr_debug("Entered, ocr focus mode %d\n", mode);
...
static int jc_set_softlanding(void)
{
	...
	for (retry = 2; retry > 0; retry--) {
		...
		if (check_softlanding == 0x30) {
			pr_debug("Finish softlanding");
			break;
		} else {
			pr_debug("softlanding status : 0x%x", check_softlanding);
			msleep(33);
		}
	}
...
static int jc_set_fps(int mode, int min, int max)
{
	...
	pr_debug("Entered, fps mode : %d, min : %d, max : %d\n",
		mode, min, max);

	if (mode == 0) {
		pr_debug("Auto fps mode\n");
...
static int jc_set_hw_vdis(void)
{
	...
	pr_debug("Entered, hw vdis mode : %d\n", jc_ctrl->hw_vdis_mode);
	...
	if (jc_ctrl->hw_vdis_mode != prev_vdis_mode) {
		...
		pr_debug("hw vdis mode same as previous mode : %d",
			prev_vdis_mode);
	return rc;
}
...
void sensor_native_control(void __user *arg)
{
	...
	if (copy_from_user(&ctrl_info,
		...
		sizeof(ctrl_info))) {
		pr_debug("fail copy_from_user!");
		goto FAIL_END;
	}

	pr_debug("sensor_native_control : %d, %d, %d, %d",
	...
	case EXT_CAM_PREVIEW_SIZE:
		pr_debug("EXT_CAM_PREVIEW_SIZE\n");
		break;

	case EXT_CAM_CAPTURE_SIZE:
		pr_debug("EXT_CAM_CAPTURE_SIZE\n");
		break;
	...
	case EXT_CAM_START_HDR:
		pr_debug("EXT_CAM_START_HDR/LLS : %d", ctrl_info.value_1);
	...
	case EXT_CAM_RESET_HDR:
		pr_debug("EXT_CAM_RESET_HDR");
		jc_writeb(0x0C, 0x0A, 0x00);
		break;

	case EXT_CAM_RESUME_PREVIEW:
		pr_debug("EXT_CAM_RESUME_PREVIEW");
	...
	case EXT_CAM_GET_ISP_DBG_LOG:
		pr_debug("EXT_CAM_GET_ISP_DBG_LOG");
		jc_get_isp_debug_log();
		break;
#endif

	case EXT_CAM_FD_MODE:
		pr_debug("EXT_CAM_FD_MODE");
		if (ctrl_info.value_1 == 0) {
			pr_debug("FD disable\n");
	...
	case EXT_CAM_START_AE_AWB_LOCK:
		pr_debug("EXT_CAM_START_AE_AWB_LOCK");
		if (ctrl_info.value_1 == 0) {
			pr_debug("AE unlock\n");
			jc_writeb(JC_CATEGORY_AE,
					0x00, 0x00);
	...
	case EXT_CAM_SET_HW_VDIS:
		pr_debug("EXT_CAM_SET_HW_VDIS: %d", ctrl_info.value_1);
...
FAIL_END:
	pr_debug("Error : can't handle native control");
...
retry:
	...
	if (isp_version == 0x00) {
		...
		err = jc_check_fw();
		if (err < 0) {
			pr_debug("Need to fw update!\n");
			firmware_update = true;
			s_ctrl->func_tbl->sensor_power_down(s_ctrl);
			goto retry;
		} else if (err > 0) {
			if (firmware_update_sdcard == true) {
				pr_debug("Force update via sdcard fw\n");
				firmware_update = true;
				s_ctrl->func_tbl->sensor_power_down(s_ctrl);
				goto retry;
			} else
				pr_debug("Sensor FW is latest!\n");
		} else
			pr_debug("Sensor FW is latest!\n");
	} else if (isp_version == 0x10) {
		if (version_checked == false && jc_ctrl->fw_update == true) {
                   ...
                   if (firmware_update_sdcard == true) {
                       pr_debug("FW in sd card is higher priority than others!\n");
                       jc_isp_boot();
                       ...
                       if (result_isp_phone != 0) {
                           pr_debug("ISP FW and SD card FW are different, Force Write!\n");
                           jc_isp_reset(s_ctrl);
                           jc_load_SIO_fw();
                           firmware_update = true;
                           recheck_fw = true;
                           ...
                           pr_debug("ISP FW and SD card FW are same, Do not anything!\n");
                   } else if (isp_ret < 0) {
                       pr_debug("ISP FW is wrong, write Phone FW to ISP\n");
                       jc_load_SIO_fw();
                       jc_phone_fw_to_isp();
                       jc_sensor_power_reset(s_ctrl);
                       jc_get_isp_version();
                       ...
                       if (result_sensor_isp > 0) {
                           pr_debug("Sensor > ISP, update from sensor\n");
                           jc_isp_reset(s_ctrl);
                           jc_load_SIO_fw();
                           firmware_update = true;
                           recheck_fw = true;
                       ...
                       pr_debug("ISP FW is normal, need to compare with sensor/phone");
                       jc_isp_boot();
                       jc_get_sensor_version();
                       ...
                       if (result_sensor_phone > 0) {
                           ...
                           if (result_isp_phone < 0) {
                               pr_debug("Phone > ISP, update from Phone\n");
                               jc_isp_reset(s_ctrl);
                               jc_load_SIO_fw();
                               firmware_update = true;
                               recheck_fw = true;
	...
	pr_debug("isp revision : 0x%x\n", isp_revision);

	pr_debug("nv12 output setting\n");
...
void jc_sensor_start_stream(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (jc_ctrl->anti_stream_off == true) {
		...
		pr_debug("change to monitor mode\n");
		jc_set_mode(JC_MONITOR_MODE);
		jc_ctrl->stream_on = true;
		if (jc_ctrl->af_mode == 3) {
			pr_debug("start CAF\n");
...
void jc_sensor_stop_stream(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (jc_ctrl->anti_stream_off == false) {
		/* change to parameter mode */
		pr_debug("change to parameter mode\n");
		jc_set_mode(JC_PARMSET_MODE);
		jc_ctrl->stream_on = false;
	}
	CAM_DEBUG("X");
...
int32_t jc_init_vreg_port(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!s_ctrl->reg_ptr) {
		pr_debug("%s: could not allocate mem for regulators\n",
			__func__);
		return -ENOMEM;
	}
	...
	rc = msm_camera_enable_vreg(
		...
		s_ctrl->reg_ptr, 1);
	if (rc < 0) {
		pr_debug("%s: enable regulator failed\n",
			__func__);
			    ...
			    const struct i2c_device_id *id)
{
	...
	rc = jc_spi_init();
	if (rc)
		pr_debug("failed to register jc fw - %x\n", rc);
	...
	if (s_ctrl->sensor_i2c_client != NULL) {
		...
		pr_debug("s_ctrl->sensor_i2c_client is NULL");
		rc = -EFAULT;
		return rc;
	}
...
static int __init jc_init(void)
{
	...
	if (IS_ERR(cam_dev_rear)) {
		pr_debug("failed to create device cam_dev_rear!\n");
		return 0;
	}
