...
void lm3533_power_suspend(struct early_suspend * h)
{
...
void lm3533_power_resume(struct power_suspend * h)
{
...
#if defined(CONFIG_FB_MSM_MIPI_LGIT_CMD_WVGA_INVERSE_PT_PANEL) || \
	defined(CONFIG_FB_MSM_MIPI_LGIT_VIDEO_WVGA_INVERSE_PT_PANEL)
	power_suspend.suspend = lm3533_early_suspend;
	power_suspend.resume = lm3533_power_resume;
#else
