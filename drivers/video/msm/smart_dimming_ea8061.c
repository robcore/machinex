...
static const unsigned char *gamma_300cd_list[GAMMA_300CD_MAX_EA8061] = {
	gamma_300cd_00,
	gamma_300cd_00,
	gamma_300cd_02,
	gamma_300cd_02,
...
static const unsigned char gamma_id_list[GAMMA_300CD_MAX_EA8061] = {
...
int init_table_info_ea8061(struct str_smart_dim *smart)
{
	...
	for (i = 0; i < GAMMA_300CD_MAX_EA8061; i++) {
		if (smart->panelid[2] == gamma_id_list[i])
			break;
	}

	if (i >= GAMMA_300CD_MAX_EA8061) {
		printk(KERN_ERR "[SMART DIMMING-WARNING] %s Can't found default gamma table\n", __func__);
		smart->default_gamma = gamma_300cd_list[GAMMA_300CD_MAX_EA8061-1];
	} else
