	...
	struct ethtool_drvinfo *info)
{
	...
	strncpy(info->driver, IOC3_NAME, sizeof(info->driver));
	strncpy(info->version, IOC3_VERSION, sizeof(info->version));
	strncpy(info->bus_info, pci_name(ip->pdev), sizeof(info->bus_info));
}
