...
{
	wl_event_msg_t msg;
	struct bcm_cfg80211 *cfg = from_timer(cfg, t, listen_timer);
	CFGP2P_DBG((" Enter\n"));
...
#define EXTRA_DELAY_TIME	100
	...
	timer_setup(&cfg->listen_timer, wl_cfgp2p_listen_expired, 0);
	cfg->listen_timer.expires = jiffies + msecs_to_jiffies(duration_ms + extra_delay);
	add_timer(&cfg->listen_timer);
#ifdef WL_CFG80211_VSDB_PRIORITIZE_SCAN_REQUEST
	wl_clr_p2p_status(cfg, LISTEN_EXPIRED);
#endif /* WL_CFG80211_VSDB_PRIORITIZE_SCAN_REQUEST */
