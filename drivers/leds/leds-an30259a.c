...
static void an30259a_start_led_pattern(unsigned int mode)
{
	...
	case CHARGING:
		...
		case BATTERY_MID:
				if (breathing_leds) {
					leds_on(LED_R, true, true, 0xAF);
					leds_on(LED_G, true, true, 0xC8);
					leds_set_slope_mode(client, LED_R,
							0, 15, 10, 0, 2, 2, 1, 1, 1, 1);
					leds_set_slope_mode(client, LED_G,
							0, 15, 10, 0, 2, 2, 1, 1, 1, 1);
				} else {
					leds_on(LED_R, true, false, 0xAF);
					leds_on(LED_G, true, false, 0xC8);
				}
				break;
	...
	case POWERING:
		...
		if (!booted) {
			...
			leds_set_slope_mode(client, LED_R,
					0, 15, 0, 0, 4, 4, 1, 0, 2, 1);
			leds_set_slope_mode(client, LED_G,
					0, 15, 15, 10, 4, 4, 1, 1, 1, 1);
			leds_set_slope_mode(client, LED_B,
					4, 15, 0, 0, 4, 4, 1, 2, 0, 1);
			booted = true;
		} else {
