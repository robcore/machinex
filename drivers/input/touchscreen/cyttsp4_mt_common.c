...
static void cyttsp4_mt_power_suspend(struct power_suspend *h)
{
...
static void cyttsp4_mt_power_resume(struct power_suspend *h)
{
	struct cyttsp4_mt_data *md =
		container_of(h, struct cyttsp4_mt_data, es);
	struct device *dev = &md->ttsp->dev;
...
void cyttsp4_setup_power_suspend(struct cyttsp4_mt_data *md)
{
//	md->es.level = POWER_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	md->es.suspend = cyttsp4_mt_power_suspend;
	md->es.resume = cyttsp4_mt_power_resume;
...
#ifdef CONFIG_POWERSUSPEND
	cyttsp4_setup_power_suspend(md);
#endif
