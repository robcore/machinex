...
/**
 * for_each_cpu_but_zero - iterate over every cpu in a mask except zero
 * @cpu: the (optionally unsigned) integer iterator
 * @mask: the cpumask pointer
 *
 * After the loop, cpu is >= nr_cpu_ids.
 */
#define for_each_cpu_but_zero(cpu, mask)				\
	for ((cpu) = 0;				\
		(cpu) = cpumask_next((cpu), (mask)),	\
		(cpu) < nr_cpu_ids;)
...
/**
 * for_each_cpu_not_but_zero - iterate over every cpu except boot in a complemented mask
 * @cpu: the (optionally unsigned) integer iterator
 * @mask: the cpumask pointer
 *
 * After the loop, cpu is >= nr_cpu_ids.
 */
#define for_each_cpu_not_but_zero(cpu, mask)				\
	for ((cpu) = 0;					\
		(cpu) = cpumask_next_zero((cpu), (mask)),	\
		(cpu) < nr_cpu_ids;)
...
#define for_each_possible_cpu_reverse()				\
	unsigned int cpu;				\
	for (cpu = 3; cpu > 0; cpu--);
...
#define cpu_out_of_range(cpu) ((cpu) < 0 || (cpu) > (NR_CPUS - 1))
#define cpu_in_range_hp(cpu) ((cpu) >= 1 && (cpu) <= (NR_CPUS - 1))
#define cpu_out_of_range_hp(cpu) ((cpu) < 1 || (cpu) > (NR_CPUS - 1))
