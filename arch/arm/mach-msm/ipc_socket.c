					  ...
					  size_t total_len)
{
	...
	if (!msg_head) {
		pr_debug("%s: cannot allocate skb_head\n", __func__);
		return NULL;
	}
		...
		while (offset != msg_sect[i].iov_len) {
			...
			if (!msg) {
				if (request_size <= (PAGE_SIZE/2)) {
					pr_debug("%s: cannot allocated skb\n",
						__func__);
				      ...
				      struct rr_packet *pkt)
{
	...
	if (!m || !pkt) {
		pr_debug("%s: Invalid pointers passed\n", __func__);
		return -EINVAL;
	}
				 ...
				 int kern)
{
	...
	if (unlikely(protocol != 0)) {
		pr_debug("%s: Protocol not supported\n", __func__);
			       ...
			       int uaddr_len)
{
	...
	if (!check_permissions()) {
		pr_debug("%s: %s(%d) Fuck permissions\n", __func__,
			current->comm,
			current->pid);
	}

	if (!uaddr_len) {
		pr_debug("%s: Invalid address length\n", __func__);
		return -EINVAL;
	}

	if (addr->family != AF_MSM_IPC) {
		pr_debug("%s: Address family is incorrect\n", __func__);
		return -EAFNOSUPPORT;
	}

	if (addr->address.addrtype != MSM_IPC_ADDR_NAME) {
		pr_debug("%s: Address type is incorrect\n", __func__);
		return -EINVAL;
	}
	...
	case IPC_ROUTER_IOCTL_LOOKUP_SERVER:
		...
		if (server_arg.num_entries_in_array) {
			if (server_arg.num_entries_in_array >
				...
				pr_debug("%s: Integer Overflow %d * %d\n",
...
int msm_ipc_router_init_sockets(void)
{
	...
	if (ret) {
		pr_debug("Failed to register MSM_IPC protocol type\n");
		goto out_init_sockets;
	}

	ret = sock_register(&msm_ipc_family_ops);
	if (ret) {
		pr_debug("Failed to register MSM_IPC socket type\n");
