...
int local_timer_setup(struct clock_event_device *evt)
{
	...
	if (soc_class_is_msm8x60() || soc_class_is_msm8960() ||
...
void __init msm_timer_init(void)
{
	...
	if (soc_class_is_msm7x01() || soc_class_is_msm7x25() || soc_class_is_msm7x27() ||
	    soc_class_is_msm7x25a() || soc_class_is_msm7x27a() || soc_class_is_msm7x25aa() ||
	    soc_class_is_msm7x27aa() || soc_class_is_msm8625() || soc_class_is_msm7x25ab()) {
		...
		gpt->regbase = MSM_TMR_BASE;
		dgt->regbase = MSM_TMR_BASE + 0x10;
	} else if (soc_class_is_fsm9xxx())
		dgt->freq = 4800000;
	else if (soc_class_is_msm7x30() || soc_class_is_msm8x55()) {
		gpt->status_mask = BIT(10);
		dgt->status_mask = BIT(2);
		dgt->freq = 6144000;
	} else if (soc_class_is_msm8x60()) {
		global_timer_offset = MSM_TMR0_BASE - MSM_TMR_BASE;
		gpt->status_mask = BIT(10);
		dgt->status_mask = BIT(2);
		dgt->freq = 6750000;
		...
		if (!soc_class_is_msm8930() && !soc_class_is_msm8960ab()) {
			gpt->flags |= MSM_CLOCK_FLAGS_UNSTABLE_COUNT;
			dgt->flags |= MSM_CLOCK_FLAGS_UNSTABLE_COUNT;
		}
	} else {
	...
	for (i = 0; i < ARRAY_SIZE(msm_clocks); i++) {
		...
		ce->irq = clock->irq;
		if (soc_class_is_msm8x60() || soc_class_is_msm9615() || soc_class_is_msm8625() ||
