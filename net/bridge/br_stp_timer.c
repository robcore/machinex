...
{
	timer_setup(&br->hello_timer, br_hello_timer_expired,
		      0);

	timer_setup(&br->tcn_timer, br_tcn_timer_expired,
		      0);
	...
	timer_setup(&br->gc_timer, br_fdb_cleanup, 0);
}

void br_stp_port_timer_init(struct net_bridge_port *p)
{
	timer_setup(&p->message_age_timer, br_message_age_timer_expired,
		      0);

	timer_setup(&p->forward_delay_timer, br_forward_delay_timer_expired,
		      0);

	timer_setup(&p->hold_timer, br_hold_timer_expired,
		      0);
}
