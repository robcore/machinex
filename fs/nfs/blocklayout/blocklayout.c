...
bl_submit_bio(int rw, struct bio *bio)
{
	if (bio) {
		...
		dprintk("%s submitting %s bio %u@%llu\n", __func__,
			rw == READ ? "read" : "write",
			bio->bio->bio_iter.bi_size, (unsigned long long)bio->bio->bio_iter.bi_sector);
		submit_bio(rw, bio);
	}
	return NULL;
}
				     ...
				     struct parallel_io *par)
{
	...
	if (bio) {
		bio->bio->bio_iter.bi_sector = isect - be->be_f_offset + be->be_v_offset;
		    ...
		    unsigned int offset, unsigned int len)
{
	...
	bio->bio->bio_iter.bio->bio_iter.bi_sector = isect - be->be_f_offset + be->be_v_offset;
	bio->bi_bdev = be->be_mdev;
