...
unsigned int boeffla_sound_hook_tabla_write(unsigned int reg, unsigned int value)
{
	...
	switch (reg)
	{
		case TABLA_A_CDC_RX1_VOL_CTL_B2_CTL: // headphone L
		{
			value = headphone_volume_l;
			break;
		}

		case TABLA_A_CDC_RX2_VOL_CTL_B2_CTL: // headphone R
		{
			value = headphone_volume_l;
			break;
		}

		case TABLA_A_CDC_RX5_VOL_CTL_B2_CTL: // speaker
		{
			value = speaker_volume_l;
			break;
		}

		case TABLA_A_CDC_RX7_VOL_CTL_B2_CTL: // speaker
		{
			value = speaker_volume_r;
			break;
		}

//		case TABLA_A_CDC_TX7_VOL_CTL_GAIN: // mic level general
//		{
//			value = mic_level;
//			break;
//		}
...
static void reset_audio_hub(void)
{
	...
	tabla_write_no_hook(codec, TABLA_A_CDC_RX1_VOL_CTL_B2_CTL, HEADPHONE_DEFAULT + HEADPHONE_REG_OFFSET);
	tabla_write_no_hook(codec, TABLA_A_CDC_RX2_VOL_CTL_B2_CTL, HEADPHONE_DEFAULT + HEADPHONE_REG_OFFSET);

	tabla_write_no_hook(codec, TABLA_A_CDC_RX5_VOL_CTL_B2_CTL, SPEAKER_DEFAULT + SPEAKER_REG_OFFSET);
	tabla_write_no_hook(codec, TABLA_A_CDC_RX7_VOL_CTL_B2_CTL, SPEAKER_DEFAULT + SPEAKER_REG_OFFSET);

//	tabla_write_no_hook(codec, TABLA_A_CDC_TX7_VOL_CTL_GAIN, MICLEVEL_DEFAULT + MICLEVEL_REG_OFFSET);
					...
					const char *buf, size_t count)
{
	...
	// set new values
	tabla_write_no_hook(codec, TABLA_A_CDC_RX1_VOL_CTL_B2_CTL, 
		headphone_volume_l + HEADPHONE_REG_OFFSET);
	tabla_write_no_hook(codec, TABLA_A_CDC_RX2_VOL_CTL_B2_CTL, 
					...
					const char *buf, size_t count)
{
	...
	// set new values
	tabla_write_no_hook(codec, TABLA_A_CDC_RX5_VOL_CTL_B2_CTL, 
		speaker_volume_l + SPEAKER_REG_OFFSET);
	tabla_write_no_hook(codec, TABLA_A_CDC_RX7_VOL_CTL_B2_CTL, 
					...
					const char *buf, size_t count)
{
	...
	// set new value
	tabla_write_no_hook(codec, TABLA_A_CDC_TX4_VOL_CTL_GAIN, 
...
static ssize_t register_dump_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX1_VOL_CTL_B1_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX1_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX2_VOL_CTL_B1_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX2_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX3_VOL_CTL_B1_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX3_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX4_VOL_CTL_B1_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX4_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX5_VOL_CTL_B1_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX5_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX6_VOL_CTL_B1_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX6_VOL_CTL_B1_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX7_VOL_CTL_B1_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX7_VOL_CTL_B1_CTL));

	// Output volumes Bank2
	sprintf(buf+strlen(buf), "\n** Output values bank 2\n");

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX1_VOL_CTL_B2_CTL: %d (head l)\n", 
		tabla_read(codec, TABLA_A_CDC_RX1_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX2_VOL_CTL_B2_CTL: %d (head r)\n", 
		tabla_read(codec, TABLA_A_CDC_RX2_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX3_VOL_CTL_B2_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX3_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX4_VOL_CTL_B2_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX4_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX5_VOL_CTL_B2_CTL: %d (speaker)\n", 
		tabla_read(codec, TABLA_A_CDC_RX5_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX6_VOL_CTL_B2_CTL: %d\n", 
		tabla_read(codec, TABLA_A_CDC_RX6_VOL_CTL_B2_CTL));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_RX7_VOL_CTL_B2_CTL: %d (speaker)\n", 
		tabla_read(codec, TABLA_A_CDC_RX7_VOL_CTL_B2_CTL));

	// Input gains
	sprintf(buf+strlen(buf), "\n** Input gains\n");

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX1_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX1_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX2_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX2_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX3_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX3_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX4_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX4_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX5_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX5_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX6_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX6_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX7_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX7_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX8_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX8_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX9_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX9_VOL_CTL_GAIN));

	sprintf(buf+strlen(buf), "TABLA_A_CDC_TX10_VOL_CTL_GAIN: %d\n", 
		tabla_read(codec, TABLA_A_CDC_TX10_VOL_CTL_GAIN));
