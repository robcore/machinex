				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	mxcodec_dbg("%s: ear_pa_gain = 0x%x\n", __func__, ear_pa_gain);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	mxcodec_dbg("%s: ucontrol->value.integer.value[0]  = %ld\n", __func__,
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	mxcodec_dbg("%s: IIR #%d band #%d enable %d\n", __func__,
		iir_idx, band_idx,
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	mxcodec_dbg("%s: IIR #%d band #%d b0 = 0x%x\n"
	...
	int index)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event) && (enable != 0)) {
		...
		mxcodec_dbg("%s: listed whole_db_gain hex:0x%x dec:0x%d, adjusted whole_db_gainhex:0x%x dec:0x%d\n",
			 __func__, comp_dgtl_gain[pa_gain & 0xF].whole_db_gain, comp_dgtl_gain[pa_gain & 0xF].whole_db_gain,
	...
	mxcodec_dbg("%s: half_db_gain(%d)whole_db_gain(%d)comp_gain_offset[%d](%d)\n",
					   ...
					   struct snd_ctl_elem_value *ucontrol)
{
	...
	int comp = ((struct soc_multi_mixer_control *)
					kcontrol->private_value)->shift;
	int value = ucontrol->value.integer.value[0];
	mxcodec_dbg("%s: compander #%d enable %d\n",
		 __func__, comp + 1, value);
	if (value == tabla->comp_enabled[comp]) {
		mxcodec_dbg("%s: compander #%d enable %d no change\n",
			 __func__, comp + 1, value);
		return 0;
	}
						  ...
						  int event)
{
	...
	mxcodec_dbg("%s: compander #%d enable %d event %d widget name %s\n",
		 __func__, w->shift + 1,
		 tabla->comp_enabled[w->shift], event , w->name);
	if (tabla->comp_enabled[w->shift] == 0)
		goto rtn;
	if ((w->shift == COMPANDER_1) && (tabla->anc_func)) {
		mxcodec_dbg("%s: ANC is enabled so compander #%d cannot be enabled\n",
			 __func__, w->shift + 1);
		goto rtn;
	}
	switch (event) {
						...
						int event)
{
	struct snd_soc_codec *codec = w->codec;
	struct tabla_priv *tabla = snd_soc_codec_get_drvdata(codec);
	mxcodec_dbg("%s: compander#1->enable(%d) reg(0x%x = 0x%x) event(%d)\n",
	...
	enum tabla_bandgap_type choice)
{
	...
	mxcodec_dbg("%s, choice is %d, current is %d\n", __func__, choice,
		tabla->bandgap_type);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (tabla->aux_pga_cnt++ == 1
			...
			mxcodec_dbg("AUX PGA enabled RC osc\n");
		}
		break;
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMU:
		mxcodec_dbg("%s: sleeping 16 ms after %s PA turn on\n",
				__func__, w->name);
		mdelay(16);
		break;
	case SND_SOC_DAPM_POST_PMD:
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case 2:
		...
		mxcodec_dbg("%s() event %d DMIC%d dmic_1_2_clk_cnt %d\n",
	...
	case 4:
		...
		mxcodec_dbg("%s() event %d DMIC%d dmic_3_4_clk_cnt %d\n",
			__func__, event,  dmic, *dmic_clk_cnt);
		break;
	...
	case 6:
		...
		mxcodec_dbg("%s() event %d DMIC%d dmic_5_6_clk_cnt %d\n",
			__func__, event,  dmic, *dmic_clk_cnt);
...
static void tabla_codec_start_hs_polling(struct snd_soc_codec *codec)
{
	...
	mxcodec_dbg("%s: enter\n", __func__);
	if (!tabla->mbhc_polling_active) {
		mxcodec_dbg("Polling is not active, do not start polling\n");
		return;
	}
	snd_soc_write(codec, TABLA_A_MBHC_SCALING_MUX_1, 0x84);

	if (tabla->no_mic_headset_override) {
		mxcodec_dbg("%s setting button threshold to min", __func__);
		/* set to min */
		...
		mxcodec_dbg("%s recovering MBHC state machine\n", __func__);
...
static void tabla_codec_pause_hs_polling(struct snd_soc_codec *codec)
{
	...
	mxcodec_dbg("%s: enter\n", __func__);
	if (!tabla->mbhc_polling_active) {
		mxcodec_dbg("polling not active, nothing to pause\n");
		return;
	}
...
static void tabla_codec_switch_cfilt_mode(struct snd_soc_codec *codec, int mode)
{
	...
	if (cur_mode_val != reg_mode_val) {
		...
		mxcodec_dbg("%s: CFILT mode change (%x to %x)\n", __func__,
			cur_mode_val, reg_mode_val);
	} else {
		mxcodec_dbg("%s: CFILT Value is already %x\n",
					   ...
					   int usec)
{
	...
	if (tabla->mbhc_data.micb_mv != VDDIO_MICBIAS_MV &&
		...
		mxcodec_dbg("%s: set mic V to micbias V\n", __func__);
					 ...
					 bool checkpolling)
{
	...
	if (vddio_switch && !tabla->mbhc_micbias_switched &&
		...
		if (tabla->mbhc_data.micb_mv != VDDIO_MICBIAS_MV) {
			...
			snd_soc_write(codec, TABLA_A_CDC_MBHC_VOLT_B2_CTL,
				      (tabla->mbhc_data.adj_v_ins_hu >> 8) &
				       0xFF);
			mxcodec_dbg("%s: Programmed MBHC thresholds to VDDIO\n",
				 __func__);
		}
		...
		mxcodec_dbg("%s: VDDIO switch enabled\n", __func__);
		...
		if (tabla->mbhc_data.micb_mv != VDDIO_MICBIAS_MV) {
			...
			mxcodec_dbg("%s: Programmed MBHC thresholds to MICBIAS\n",
				 __func__);
		}
		...
		mxcodec_dbg("%s: VDDIO switch disabled\n", __func__);
	}
}
...
static void tx_hpf_corner_freq_callback(struct work_struct *work)
{
	...
	mxcodec_dbg("%s(): decimator %u hpf_cut_of_freq 0x%x\n", __func__,
...
static void tx_digital_unmute_callback(struct work_struct *work)
{
	...
	mxcodec_dbg("%s(): tabla %p decimator %u tx digital mute 0\n",
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	mxcodec_dbg("%s(): widget = %s dec_name = %s decimator = %u\n", __func__,
	...
	u32 jack_status, int irq)
{
	...
	mxcodec_dbg("%s: clear ocp status %x\n", __func__, jack_status);
	codec = tabla->codec;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	mxcodec_dbg("%s: DAPM Event %d ANC func is %d\n",
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		mxcodec_dbg("%s: sleep 10 ms after %s PA disable.\n", __func__,
				w->name);
		usleep_range(10000, 10000);
		break;
	}
		...
		struct snd_soc_dai *dai)
{
	...
	mxcodec_dbg("%s(): substream = %s  stream = %d\n" , __func__,
...
int tabla_mclk_enable(struct snd_soc_codec *codec, int mclk_enable, bool dapm)
{
	...
	mxcodec_dbg("%s: mclk_enable = %u, dapm = %d\n", __func__, mclk_enable,
		 dapm);
	if (dapm)
		TABLA_ACQUIRE_LOCK(tabla->codec_resource_lock);
	if (mclk_enable) {
...

{
	...
	if (dai->id == AIF1_PB || dai->id == AIF2_PB || dai->id == AIF3_PB) {
		...
		for(i = 0; i < rx_num ; i++)
			mxcodec_dbg("[%d]",tabla->dai[dai->id - 1].ch_num[i]);
		mxcodec_dbg(" WCD channel mapping\n");
		...
		mxcodec_dbg("=[WCD]=%s: APQ<-", __func__);
		for(i = 0; i < tx_num ; i++)
		...
		for (j = 0; j < NUM_INTERPOLATORS; j++) {
			...
			if (((rx_mix_1_reg_1_val & 0x0F) == rx_mix1_inp) ||
			   ...
			   || ((rx_mix_1_reg_2_val & 0x0F) == rx_mix1_inp)) {
				...
				mxcodec_dbg("%s: %s connected to RX%u\n", __func__,
					w[i].name, j + 1);

				mxcodec_dbg("%s: set RX%u sample rate to %u\n",
	...
	u8 tx_fs_rate_reg_val, u32 sample_rate)
{
	...
	for (i = 0; i < ARRAY_SIZE(tabla_dapm_aif_out_widgets); i++) {
		...
		if (decimator) { /* SLIM_TX port has a DEC as input */
			...
			mxcodec_dbg("%s: set DEC%u (-> SLIM_TX%u) rate to %u\n",
			...
			if ((tx_port_reg_val >= 0x1) &&
				...
				mxcodec_dbg("%s: RMIX%u going to SLIM TX%u\n",
				...
				mxcodec_dbg("%s: no signal to SLIM TX%u\n",
						__func__, tx_port);
			} else {
		...
		struct snd_soc_dai *dai)
{
	...
	mxcodec_dbg("%s: dai_name = %s DAI-ID %x rate %d num_ch %d\n", __func__,
...
static short tabla_codec_setup_hs_polling(struct snd_soc_codec *codec)
{
	...
	mxcodec_dbg("%s: enter, mclk_enabled %d\n", __func__, tabla->mclk_enabled);
...
void tabla_set_and_turnoff_hph_padac(struct snd_soc_codec *codec)
{
	...
	if (tabla_is_hph_pa_on(codec)) {
		mxcodec_dbg("%s PA is on, setting PA_OFF_ACK\n", __func__);
...
static void tabla_clr_and_turnon_hph_padac(struct tabla_priv *tabla)
{
	...
	if (test_and_clear_bit(TABLA_HPHR_PA_OFF_ACK,
			       &tabla->hph_pa_dac_state)) {
		mxcodec_dbg("%s: HPHR clear flag and enable PA\n", __func__);
	...
	if (test_and_clear_bit(TABLA_HPHL_PA_OFF_ACK,
			       &tabla->hph_pa_dac_state)) {
		mxcodec_dbg("%s: HPHL clear flag and enable PA\n", __func__);
	...
	if (pa_turned_on) {
		mxcodec_dbg("%s: PA was turned off by MBHC and not by DAPM\n",
				__func__);
		usleep_range(wg_time * 1000, wg_time * 1000);
	}
}
...
{
	struct tabla_priv *tabla = snd_soc_codec_get_drvdata(codec);
	mxcodec_dbg("%s: enter insertion %d hph_status %x\n",
	...
	if (!insertion) {
		...
		if (tabla->mbhc_cfg.headset_jack) {
			...
			* report release if we reported button press */
			if (tabla_cancel_btn_work(tabla)) {
				mxcodec_dbg("%s: button press is canceled\n",
					__func__);
			} else if (tabla->buttons_pressed) {
				mxcodec_dbg("%s: Reporting release for reported "
					 "button press %d\n", __func__,
					 jack_type);
				tabla_snd_soc_jack_report(tabla,
			...
			mxcodec_dbg("%s: Reporting removal %d(%x)\n", __func__,
		...
		if (tabla->mbhc_cfg.detect_extn_cable) {
			...
			if (tabla->hph_status != jack_type &&
			    tabla->mbhc_cfg.headset_jack) {
				mxcodec_dbg("%s: Reporting removal (%x)\n",
		...
		if (tabla->mbhc_cfg.headset_jack) {
			mxcodec_dbg("%s: Reporting insertion %d(%x)\n", __func__,
				 jack_type, tabla->hph_status);
			tabla_snd_soc_jack_report(tabla,
						  tabla->mbhc_cfg.headset_jack,
						  tabla->hph_status,
	...
	mxcodec_dbg("%s: leave hph_status %x\n", __func__, tabla->hph_status);
}
					...
					bool padac_off)
{
	...
	mxcodec_dbg("%s: enter insertion(%d) trigger(0x%x)\n",
	...
	if (insertion) {
		mxcodec_dbg("%s: setup for insertion\n", __func__);
		tabla_codec_switch_micbias(codec, 0);
		...
		mxcodec_dbg("setup for removal detection\n");
	...
	if (tabla) {
		if (tabla->mbhc_cfg.button_jack) {
			...
			dce_mv = tabla_codec_sta_dce_v(tabla->codec, 1,
						bias_value);
			mxcodec_dbg("%s: Reporting long button press event"
				 " STA: %d, DCE: %d\n", __func__,
				 sta_mv, dce_mv);
			tabla_snd_soc_jack_report(tabla,
				  ...
				  const s32 micmv)
{
	...
	if (btn == -1)
		mxcodec_dbg("%s: couldn't find button number for mic mv %d\n",
			 __func__, micmv);
...
static irqreturn_t tabla_dce_handler(int irq, void *data)
{
	...
	if (priv->mbhc_state == MBHC_STATE_POTENTIAL_RECOVERY) {
		mxcodec_dbg("%s: mbhc is being recovered, skip button press\n",
			 __func__);
		goto done;
	}
	...
	if (priv->in_gpio_handler) {
		mxcodec_dbg("%s: GPIO State Changed, ignore button press\n",
			 __func__);
		btn = -1;
		goto done;
	}
	...
	if (mbhc_status != TABLA_MBHC_STATUS_REL_DETECTION) {
		if (priv->mbhc_last_resume &&
			...
			mxcodec_dbg("%s: Button is already released shortly after "
				 "resume\n", __func__);
			n_btn_meas = 0;
		}
	}
	...
	mxcodec_dbg("%s: meas HW - DCE %x,%d,%d button %d\n", __func__,
	...
	if (n_btn_meas == 0) {
		...
		mxcodec_dbg("%s: meas HW - STA %x,%d,%d button %d\n", __func__,
	...
	for (; ((d->n_btn_meas) && (meas < (d->n_btn_meas + 1))); meas++) {
		...
		btnmeas[meas] = tabla_determine_button(priv, mv_s);
		mxcodec_dbg("%s: meas %d - DCE %x,%d,%d button %d\n",
	...
	if (btn >= 0) {
		if (priv->in_gpio_handler) {
			mxcodec_dbg("%s: GPIO already triggered, ignore button "
				 "press\n", __func__);
			goto done;
		}
		...
		mxcodec_dbg("%s: bogus button press, too short press?\n",
			 __func__);
	}
...
static irqreturn_t tabla_release_handler(int irq, void *data)
{
	...
	if (priv->buttons_pressed & TABLA_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			mxcodec_dbg("%s: Reporting long button release event\n",
				 __func__);
			...
			if (tabla_is_fake_press(priv)) {
				mxcodec_dbg("%s: Fake button press interrupt\n",
					 __func__);
			} else if (priv->mbhc_cfg.button_jack) {
				if (priv->in_gpio_handler) {
					mxcodec_dbg("%s: GPIO kicked in, ignore\n",
						 __func__);
				} else {
					mxcodec_dbg("%s: Reporting short button "
						 "press and release\n",
						 __func__);
					tabla_snd_soc_jack_report(priv,
				...
				enum tabla_mbhc_plug_type plug_type)
{
	...
	mxcodec_dbg("%s: enter current_plug(%d) new_plug(%d)\n",
	...
	if (plug_type == PLUG_TYPE_HEADPHONE &&
		...
		if (tabla->mbhc_cfg.detect_extn_cable) {
			...
			tabla_codec_cleanup_hs_polling(codec);
			mxcodec_dbg("%s: setup mic trigger for further detection\n",
				 __func__);
			...
			if (tabla->current_plug == PLUG_TYPE_NONE)
				tabla_codec_report_plug(codec, 1,
							SND_JACK_HEADPHONE);
			tabla_codec_cleanup_hs_polling(codec);
			mxcodec_dbg("setup mic trigger for further detection\n");
			tabla->lpi_enabled = true;
			tabla_codec_enable_hs_detect(codec, 1,
						     MBHC_USE_MB_TRIGGER |
...
{
	mxcodec_dbg("%s: scheduling tabla_hs_correct_gpio_plug\n", __func__);
...
static void tabla_cancel_hs_detect_plug(struct tabla_priv *tabla,
		struct work_struct *correct_plug_work)
{
	mxcodec_dbg("%s: canceling hs_correct_plug_work\n", __func__);
	tabla->hs_detect_work_stop = true;
	wmb();
	TABLA_RELEASE_LOCK(tabla->codec_resource_lock);
	if (cancel_work_sync(correct_plug_work)) {
		mxcodec_dbg("%s: hs_correct_plug_work is canceled\n", __func__);
...
tabla_codec_get_plug_type(struct snd_soc_codec *codec, bool highhph)
{
	...
	for (i = 0; i < num_det; i++) {
		...
		mxcodec_dbg("%s: DCE #%d, %04x, V %d, scaled V %d, GND %d, "
			 "VDDIO %d, inval %d\n", __func__,
	...
	for (i = 0; (plug_type[0] != PLUG_TYPE_GND_MIC_SWAP && !inval) &&
		...
		if (mic_mv[i] < plug_type_ptr->v_no_mic) {
			...
			mxcodec_dbg("%s: Detect attempt %d, detected Headphone\n",
				 __func__, i);
		} else if (highhph && (mic_mv[i] > plug_type_ptr->v_hs_max)) {
			plug_type[i] = PLUG_TYPE_HIGH_HPH;
			mxcodec_dbg("%s: Detect attempt %d, detected High "
				 "Headphone\n", __func__, i);
		} else {
			plug_type[i] = PLUG_TYPE_HEADSET;
			mxcodec_dbg("%s: Detect attempt %d, detected Headset\n",
				 __func__, i);
		}
	...
	mxcodec_dbg("%s: Detected plug type %d\n", __func__, plug_type[0]);
	mxcodec_dbg("%s: leave\n", __func__);
	return plug_type[0];
}
...
static void tabla_hs_correct_gpio_plug(struct work_struct *work)
{
	...
	while (!time_after(jiffies, timeout)) {
		...
		mxcodec_dbg("%s: attempt(%d) current_plug(%d) new_plug(%d)\n",
			 __func__, retry, tabla->current_plug, plug_type);
		if (plug_type == PLUG_TYPE_INVALID) {
			mxcodec_dbg("Invalid plug in attempt # %d\n", retry);
			if (!tabla->mbhc_cfg.detect_extn_cable &&
			...
			mxcodec_dbg("Good headphone detected, continue polling mic\n");
			if (tabla->mbhc_cfg.detect_extn_cable) {
			...
			TABLA_RELEASE_LOCK(tabla->codec_resource_lock);
			mxcodec_dbg("Attempt %d found correct plug %d\n", retry,
				 plug_type);
			correction = true;
			break;
		}
	...
	mxcodec_dbg("%s: leave current_plug(%d)\n",
...
static void tabla_codec_decide_gpio_plug(struct snd_soc_codec *codec)
{
	...
	if (tabla_hs_gpio_level_remove(tabla)) {
		mxcodec_dbg("%s: GPIO value is low when determining plug\n",
			 __func__);
		return;
	}
	...
	if (plug_type == PLUG_TYPE_INVALID ||
		...
		mxcodec_dbg("%s: Valid plug found, determine plug type %d\n",
			 __func__, plug_type);
		tabla_find_plug_and_report(codec, plug_type);
	}
	mxcodec_dbg("%s: leave\n", __func__);
...
static void tabla_codec_detect_plug_type(struct snd_soc_codec *codec)
{
	...
	if (tabla->mbhc_cfg.gpio) {
		...
		tabla_turn_onoff_override(codec, false);
		if (tabla_hs_gpio_level_remove(tabla))
			mxcodec_dbg("%s: GPIO value is low when determining "
				 "plug\n", __func__);
		else
	...
	if (plug_type == PLUG_TYPE_INVALID) {
		mxcodec_dbg("%s: Invalid plug type detected\n", __func__);
		snd_soc_update_bits(codec, TABLA_A_CDC_MBHC_B1_CTL, 0x02, 0x02);
		tabla_codec_cleanup_hs_polling(codec);
		tabla_codec_enable_hs_detect(codec, 1,
					     MBHC_USE_MB_TRIGGER |
		...
		mxcodec_dbg("%s: GND-MIC swapped plug type detected\n", __func__);
		tabla_codec_report_plug(codec, 1, SND_JACK_UNSUPPORTED);
		tabla_codec_cleanup_hs_polling(codec);
		tabla_codec_enable_hs_detect(codec, 0, 0, false);
	} else if (plug_type == PLUG_TYPE_HEADPHONE) {
		mxcodec_dbg("%s: Headphone Detected\n", __func__);
		tabla_codec_report_plug(codec, 1, SND_JACK_HEADPHONE);
		tabla_codec_cleanup_hs_polling(codec);
		tabla_schedule_hs_detect_plug(tabla,
					&tabla->hs_correct_plug_work_nogpio);
	} else if (plug_type == PLUG_TYPE_HEADSET) {
		mxcodec_dbg("%s: Headset detected\n", __func__);
		tabla_codec_report_plug(codec, 1, SND_JACK_HEADSET);
		...
		mxcodec_dbg("%s: High impedance plug type detected\n", __func__);
		tabla_codec_report_plug(codec, 1, SND_JACK_LINEOUT);
...
static void tabla_hs_insert_irq_gpio(struct tabla_priv *priv, bool is_removal)
{
	...
	if (!is_removal) {
		mxcodec_dbg("%s: MIC trigger insertion interrupt\n", __func__);
		...
		rmb();
		if (!priv->lpi_enabled) {
			mxcodec_dbg("%s: lpi is disabled\n", __func__);
		} else if (gpio_get_value_cansleep(priv->mbhc_cfg.gpio) ==
			   priv->mbhc_cfg.gpio_level_insert) {
			mxcodec_dbg("%s: Valid insertion, "
				 "detect plug type\n", __func__);
			tabla_codec_decide_gpio_plug(codec);
		} else {
			mxcodec_dbg("%s: Invalid insertion, "
				 "stop plug detection\n", __func__);
		}
	} else if (tabla->mbhc_cfg.detect_extn_cable) {
		mxcodec_dbg("%s: Removal\n", __func__);
		if (!tabla_hs_gpio_level_remove(tabla)) {
			/*
				       ...
				       bool is_mb_trigger)
{
	...
	if (is_removal) {
		...
		mxcodec_dbg("%s: Waiting for Headphone left trigger\n",
			__func__);
		wcd9xxx_lock_sleep(core);
		...
		if (ret != 0) {
			mxcodec_dbg("%s: Complete plug insertion, Detecting plug "
				 "type\n", __func__);
				     ...
				     bool is_mb_trigger)
{
	...
	if (is_mb_trigger) {
		...
		mxcodec_dbg("%s: HPHL trigger received, detecting plug type\n",
			__func__);
		tabla_codec_detect_plug_type(codec);
	}
}
...
static bool tabla_hs_remove_settle(struct snd_soc_codec *codec)
{
	...
	while (!(timedout = time_after(jiffies, timeout))) {
		...
		if (tabla->mbhc_cfg.gpio && tabla_hs_gpio_level_remove(tabla)) {
			mxcodec_dbg("%s: GPIO indicates removal\n", __func__);
			break;
		}
		...
		for (i = 0; i < MBHC_NUM_DCE_PLUG_DETECT; i++) {
			...
			mic_mv[i] = tabla_codec_sta_dce_v(codec, 1 , mb_v[i]);
			mxcodec_dbg("%s : DCE run %lu, mic_mv = %d(%x)\n",
		...
		if (tabla->current_plug == PLUG_TYPE_NONE) {
			mxcodec_dbg("%s : headset/headphone is removed\n",
				 __func__);
			break;
		}
		...
		if (i == MBHC_NUM_DCE_PLUG_DETECT) {
			mxcodec_dbg("%s: MIC voltage settled\n", __func__);
			settled = true;
			msleep(200);
			break;
		}
		...
		if (!tabla->mbhc_cfg.gpio) {
			for (i = 0; i < MBHC_NUM_DCE_PLUG_DETECT; i++)
				if (mic_mv[i] < v_hs_max)
					break;
			if (i == MBHC_NUM_DCE_PLUG_DETECT) {
				mxcodec_dbg("%s: Headset is removed\n", __func__);
				break;
			}
		}
	}
	...
	if (timedout)
		mxcodec_dbg("%s: Microphone did not settle in %d seconds\n",
...
static void tabla_hs_remove_irq_nogpio(struct tabla_priv *priv)
{
	...
	if (priv->current_plug != PLUG_TYPE_HEADSET) {
		mxcodec_dbg("%s(): Headset is not inserted, ignore removal\n",
			 __func__);
...
static irqreturn_t tabla_hs_remove_irq(int irq, void *data)
{
	struct tabla_priv *priv = data;
	bool vddio;
	mxcodec_dbg("%s: enter, removal interrupt\n", __func__);
...
static void tabla_hs_correct_plug_nogpio(struct work_struct *work)
{
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (plug_type == PLUG_TYPE_HIGH_HPH
			 ...
			 */
			mxcodec_dbg("%s(): Plug may be removed, setup removal\n",
					 __func__);
			break;
		} else if (plug_type == PLUG_TYPE_HEADSET) {
			...
			tabla_codec_start_hs_polling(codec);
			TABLA_RELEASE_LOCK(tabla->codec_resource_lock);
			mxcodec_dbg("%s(): corrected from headphone to headset\n",
					 __func__);
			break;
		}
	}
	...
	tabla->mbhc_cfg.mclk_cb_fn(codec, 0, false);
	if (!is_headset) {
		mxcodec_dbg("%s: Inserted headphone is not a headset\n",
			__func__);
		tabla_turn_onoff_override(codec, false);
		tabla_codec_cleanup_hs_polling(codec);
		tabla_codec_enable_hs_detect(codec, 0, 0, false);
		    ...
		    const struct tabla_mbhc_config *cfg)
{
	...
	if (cfg->mclk_rate != TABLA_MCLK_RATE_12288KHZ) {
		if (cfg->mclk_rate == TABLA_MCLK_RATE_9600KHZ)
			mxcodec_dbg("Error: clock rate %dHz is not yet supported\n",
			       cfg->mclk_rate);
		else
			mxcodec_dbg("Error: unsupported clock rate %d\n",
			       cfg->mclk_rate);
		return -EINVAL;
	}
		...
		for_each_set_bit(j, &slimbus_value, BITS_PER_BYTE) {
			...
			if (val & 0x2)
				pr_err_ratelimited("underflow error on port %x,"
					" value %x\n", i*8 + j, val);
			if (val & 0x4) {
				mxcodec_dbg("%s: port %x disconnect value %x\n",
					__func__, i*8 + j, val);
				port_id = i*8 + j;
...
#ifdef CONFIG_SOUND_CONTROL
	pr_info("Tabla Sound Engine Probe\n");
...
static int tabla_resume(struct device *dev)
{
	...
	if (tabla) {
		...
		if (tabla->gpio_irq_resend) {
			...
			mxcodec_dbg("%s: Resending GPIO IRQ %d\n", __func__, irq);
			irq_set_pending(irq);
